...
def write_letter(arm:AlfredAPI, path:TrajectoryPath, init_pos:np.ndarray, z_offset:float)->None:
    coords=path.get_points()
    for coord in coords:
        if coord[-1]=='M':
            arm.set_position(z=z_offset,wait=True,speed=50,relative=True) #move up
            arm.set_position(x=init_pos[0]+coord[0],y=init_pos[1]+coord[1],wait=True,speed=100)
            arm.set_position(z=coord[2],speed=30, wait = True)
            continue
        arm.set_position(x=init_pos[0]+coord[0],y=init_pos[1]+coord[1], z=coord[2],wait=False,speed=30)
    arm.set_position(z=z_offset,wait=True,speed=50,relative=True) #move up


def write_from_file(arm:AlfredAPI, filename:str, init_pos:np.ndarray, pen_tip_height:float, cartesian_equation:np.ndarray, z_offset:float)->None:
...
def write_demo(word:str):
    arm = AlfredAPI()
    init_pos = np.array([35, 200,170, 180, -54.1, 77.6])
    path = "src/applications/modules/handwriting/svg/"+word[0].lower()+".svg"
    z_offset=10
    pen_tip_height=-46
    pen_tip_height=40
