...
def aruco_detect(
    ...
    mtx: Any,
    dist: Any,
    aruco_detect: DetectFlag = None,
) -> Tuple[Any, Any, np.array, np.array, DetectFlag]:
    ...
    corners, ids, rejected_img_points = aruco.detectMarkers(
        gray, aruco_dict, parameters=parameters, cameraMatrix=mtx, distCoeff=dist
    )
    nb_aruco_detected = 0
...
def track_aruco(
    aruco_dict_type: aruco = aruco.DICT_6X6_250,
    ids_aruco: List[int] = [3],
    ...
    # Init scanner thread
    arm = AlfredAPI()
    arm.set_servo_angle(angle=start_pose_degrees, is_radian=False, wait=True)
    scanner_thread = ScannerThread(arm, min_angle, max_angle, daemon=True)
    ...
    while True:
        ...
        if detect_aruco:
            scanner_thread.set()
            count += 1
            xy_diff = get_xy_diff(list(detect_aruco.target_dict.values()))
