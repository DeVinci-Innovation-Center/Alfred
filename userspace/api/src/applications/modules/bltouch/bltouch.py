...
def research_surface(research: str = "linear") -> float:
    ...
    return 1.0  # arm.get_position()[1][2]


def research_linear(
    arm: AlfredAPI, precisionStep: float = 0.5
...
def research_logarithmic(precisionRate: float = 0.5, maxMove: float = 4.0) -> None:
...
def research_minmax(minMove: float = 0.1, maxMove: float = 4.0) -> None:
    data = None
    p = rc.redis_instance.pubsub()
    p.subscribe("device-data-bltouch")
    send_command()
...
def recolt_data_table(
    ...
    if not (
        (compare_list(path[0], init_pos[:3]) and not relative)
        or (compare_list(path[0], np.array([0] * 3)) and relative)
    ):
        return None
    init_pos[2] += height
    arm.set_position(
...
def main():
    arm = AlfredAPI()
    init_pos = np.array([70, 320, 195.0, 180, 0.0, 90.0])
    path = np.array(
        [[70, 320, 195.0], [70, 520, 195], [210, 520, 195], [210, 320, 195]]
    )
    # z=50
    # init_pos[2]+=z
    # for Z in path:
