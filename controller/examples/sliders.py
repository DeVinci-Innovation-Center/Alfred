...
p.connect(
    p.GUI
)  # , options="--background_color_red=1.0 --background_color_blue=1.0 --background_color_green=1.0")
...
table_pos = [0, 0, -0.625]
table = p.loadURDF(
    "table/table.urdf", table_pos, flags=flags, useFixedBase=useFixedBase
)
...
while 1:
    ...
    for i in range(len(paramIds)):
        ...
        p.setJointMotorControl2(
            xarm, jointIds[i], p.POSITION_CONTROL, targetPos, force=5 * 240.0
        )
    skip_cam_frames -= 1
